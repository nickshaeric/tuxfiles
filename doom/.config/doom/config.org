...
- [[#pdf-tools][PDF Tools]]
  - [[#pdf-display-settings][PDF Display Settings]]
  - [[#pdf-keybindings][PDF Keybindings]]
...
Some macOS specific settings for better integration.

For thinner fonts on macOS, run in terminal:
...
Uses spaces over tabs globally, defaulting to 4-space width.
...
Manual-trigger completion to prevent child frames from stealing focus under AeroSpace.
...
Show LaTeX symbols in the source buffer.
